...
def xat(request):
    ...
    return render(request, 'polls/xat.html', context)
...
def xatsEnguany(request, year):
    try:
        year = date.today().year
        xats = Xat.objects.filter(date_sent__year=year)
    except Xat.DoesNotExist:
...
def xatsEnguanyMes(request, year, month):
    try:
        ...
        raise Http404("Nota does not exist")
    return render(request, 'polls/enguanymes.html', {'year':year, 'month': month, 'xats': xats})
